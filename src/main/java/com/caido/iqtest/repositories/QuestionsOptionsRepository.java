...
public interface QuestionsOptionsRepository extends JpaRepository<QuestionsOptions, Long> {
     ...
     */
    @Transactional 
    @Query("SELECT c FROM QuestionsOptions c WHERE c.idQuestions.id = :idQuestion ORDER BY c.id DESC")
    List<QuestionsOptions> findByQuestionId(@Param("idQuestion") Long idQuestion);
}
